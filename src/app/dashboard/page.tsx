...
import {
  UserCircle, Cog, Bell, Trophy, MessageCircle,
...
const ProjectPopup = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-4 rounded-lg shadow-2xl w-96 border border-gray-200">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            ...
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)}
              className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500 bg-gray-50" required />
          </div>
          <div>
          ...
          <div>
            <label className="flex items-center text-sm text-gray-700">
              <input type="checkbox" checked={lookingForMembers} onChange={(e) => setLookingForMembers(e.target.checked)}
          ...
          <div className="flex justify-end gap-2">
            <button type="button" onClick={onClose}
              className="px-4 py-1 bg-gray-200 rounded-md hover:bg-gray-300 text-gray-800 font-semibold">Cancel</button>
            <button type="submit"
              className="px-4 py-1 bg-blue-600 text-white rounded-md hover:bg-blue-700 font-semibold">Create</button>
          </div>
        </form>
      </div>
    </div>
...
export default function Dashboard() {
        ...
        <div key={idx}
          ...
          {item.label === "Settings" && showSettingsDropdown && (
            <div className="absolute left-0 mt-2 w-48 bg-white shadow-lg rounded-md border border-gray-200 z-50">
              <button onClick={() => { console.log("Toggled theme"); setShowSettingsDropdown(false); }}
              ...
              <button onClick={() => { router.push("/faq"); setShowSettingsDropdown(false); }}
  ...
  const MenuIcon = ({ onClick }: { onClick: () => void }) => (
    <button onClick={onClick}>
      <svg className="w-6 h-6 text-black" xmlns="http://www.w3.org/2000/svg"
    ...
    <div className="flex min-h-screen w-full relative">
      ...
      {showSidebar && (
        <>
          ...
          <aside className="fixed top-0 left-0 w-64 h-screen bg-gray-200 p-6 space-y-4 border-r border-gray-300 z-50 overflow-y-auto">
      ...
      <main className="flex-1 flex flex-col items-center justify-start p-8 w-full">
        <div className="w-full flex justify-between items-center mb-6 sm:mb-8 border-b border-gray-300 pb-2">
          <input type="text"
            ...
            placeholder="Search for projects..." value={searchText}
